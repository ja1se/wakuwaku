    ...
    return (
      <div className={twMerge(
        ...
        <div className="absolute top-3 left-3 lg:top-4 lg:left-4 bg-orange-400 px-1.5 py-0.5 lg:px-2 lg:py-1 rounded-[4px] z-20">
          <span className="text-slate-950 text-[10px] lg:text-[12px] font-bold uppercase tracking-tight">Exclusive</span>
        </div>
        <div className="absolute bottom-3 left-3 lg:bottom-4 lg:left-4 z-20">
          <h3 className="text-white text-base lg:text-xl font-bold drop-shadow-md group-hover:text-orange-400 transition-colors">{title}</h3>
        </div>
      </div>
    );
  }
        ...
        <div className="relative h-[112px] lg:h-[142px] overflow-hidden">
          {(!backdropUrl || imgError) ? (
            <div className="w-full h-full bg-slate-900 flex items-center justify-center p-4 text-center">
               <span className="text-slate-500 font-bold text-[10px] lg:text-xs line-clamp-2">{title}</span>
            </div>
          ) : (
            <img
              src={backdropUrl}
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center">
              <FontAwesomeIcon icon={faPlay} className="text-white text-xs lg:text-sm ml-1" />
            </div>
          </div>
        </div>
